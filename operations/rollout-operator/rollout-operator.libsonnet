...

{
  ...
  _config+:: {
    // TODO mimir $._config.multi_zone_ingester_enabled || $._config.multi_zone_store_gateway_enabled || $._config.cortex_compactor_concurrent_rollout_enabled || $._config.ingest_storage_ingester_autoscaling_enabled ||
    rollout_operator_enabled: false,
    ...
    // TODO mimir would have this = $._config.ingest_storage_ingester_autoscaling_enabled || $._config.ingester_automated_downscale_v2_enabled
    ...
    // toggle for including the custom resource definiton for the zpdb
  ...
  local enableWebhooks =
    $._config.enable_zpdb ||
    $._config.rollout_operator_replica_template_access_enabled ||
    $._config.enable_rollout_operator_webhook,

  local enableRolloutOperator = enableWebhooks || $._config.rollout_operator_enabled,
  ...
  local disable_eviction_webhook =
    if $._config.disable_rollout_operator_eviction_webhook && $._config.enable_zpdb then
      error 'disable_rollout_operator_eviction_webhook can not be used when a zpdb configuration is required'
    else
      !enableWebhooks ||
      $._config.disable_rollout_operator_eviction_webhook,


  zpdb_template:: std.parseYaml(importstr 'crds/zone-aware-pod-disruption-budget-crd.yaml'),
  zpdb_custom_resource: if !enableWebhooks || !$._config.zpdb_custom_resource_definition_enabled then null else std.parseYaml(importstr 'crds/zone-aware-pod-disruption-budget-crd.yaml'),

  replica_template:: std.parseYaml(importstr 'crds/replica-templates.yaml'),
  replica_template_custom_resource: if !enableWebhooks || !$._config.replica_template_custom_resource_definition_enabled then null else $.replica_template,
  ...
  // TODO in Mimir $.tracing_env_mixin,

  rollout_operator_deployment: if !enableRolloutOperator then null else
  ...
  rollout_operator_role: if !enableRolloutOperator then null else
  ...
  rollout_operator_rolebinding: if !enableRolloutOperator then null else
  ...
  rollout_operator_service_account: if !enableRolloutOperator then null else
