...
func TestExpiringCertificate(t *testing.T) {
	...
	// Check certificate expiration and renewal.
	{
		...
		deployment.Spec.Template.Spec.Containers[0].Args = filter(deployment.Spec.Template.Spec.Containers[0].Args, "-server-tls.self-signed-cert.expiration=10s")
		...
		require.Eventually(t, func() bool {
			...
			return true
		}, 45*time.Second, 500*time.Millisecond, "can't update rollout-operator deployment")
