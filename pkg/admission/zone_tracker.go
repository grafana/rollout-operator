...
func (zt *zoneTracker) prepareDownscale(ctx context.Context, l log.Logger, ar v1.AdmissionReview, api kubernetes.Interface, client httpClient) *v1.AdmissionResponse {
	...
	if rolloutGroup != "" {
		...
		lockedFor, alreadyLocked := zt.rolloutGroupDownscalingInProgress.LoadOrStore(rolloutGroup, ar.Request.Name)
		if alreadyLocked {
		...
		defer zt.rolloutGroupDownscalingInProgress.Delete(rolloutGroup)
	}
